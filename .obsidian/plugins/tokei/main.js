...
var ClockView = class extends import_obsidian.ItemView {
  ...
  initClockDisplay() {
    ...
    if (this.plugin.settings.showDate) {
      this.dateContainer = this.timeDateContainer.createDiv("date-container");
    ...
    this.plugin.settings.timeZonePairs.forEach((entry) => {
      const timezonePair = this.timezoneContainer.createDiv("timezone-pair");
  ...
  displayTime() {
    ...
    if (this.plugin.settings.showDate) {
      ...
      if (this.plugin.settings.showWeekAndQuarter) {
        ...
        this.weekQuarterContainer.style.display = "";
      } else {
        this.weekQuarterContainer.style.display = "none";
      }
    }
    if (this.plugin.settings.showTimeZone) {
      if (this.timezoneContainer.style.display === "none") {
        this.timezoneContainer.style.display = "";
      }
      this.updateTimezones();
    } else {
      this.timezoneContainer.style.display = "none";
    }
  }
  updateTimezones() {
    this.plugin.settings.timeZonePairs.forEach((entry) => {
  ...
  calculateWeekOfQuarter() {
    const startOfQuarter = DateTime.local().startOf("quarter");
    const currentDate = DateTime.local();
    const millisecondsPerWeek = 7 * 24 * 60 * 60 * 1e3;
    if (this.plugin.settings.weekStart === "sunday") {
      ...
      const diffInWeeks = Math.floor(
        currentDate.diff(adjustedStartOfQuarterSunday, "milliseconds").milliseconds / millisecondsPerWeek
      );
  ...
  calculateYear() {
    ...
    const month = today.getMonth();
    const yearOffset = Math.floor((month - (fiscalYearStart % 12 || 12)) / 12) + 1;
...
var ClockPlugin = class extends import_obsidian.Plugin {
  ...
  sortTimeZones() {
    ...
    return this.settings.timeZonePairs.slice().sort((a, b) => {
      const offsetA = parseFloat(a.offset);
      const offsetB = parseFloat(b.offset);
      return Math.abs(currentOffset - offsetA) - Math.abs(currentOffset - offsetB);
    });
  }
};
...
var ClockSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    const timeFormatInput = new import_obsidian.Setting(containerEl).setName("Time format").setDesc("Default Luxon format is HH:mm").addText((text) => {
      const textField = text.setPlaceholder("Enter the time format").setValue(this.plugin.settings.timeFormat);
      const hint = document.createElement("small");
      hint.classList.add("small-hint");
      textField.inputEl.after(hint);
      const handleInputDebounced = (0, import_obsidian.debounce)(() => {
      ...
      (button) => button.setIcon("reset").setTooltip("Reset time format").onClick(async () => {
        ...
        this.plugin.view.displayTime();
        const timeFormatInputEl = timeFormatInput.settingEl.querySelector("input");
        const hint = timeFormatInput.settingEl.querySelector("small");
        if (timeFormatInputEl instanceof HTMLInputElement) {
          timeFormatInputEl.value = this.plugin.settings.timeFormat;
        }
        if (hint instanceof HTMLElement) {
          hint.textContent = "";
        }
        new import_obsidian.Notice("Time Format Reset");
      })
    );
    timeFormatInput.settingEl.classList.add("time-format-settings");
    const showDateSetting = new import_obsidian.Setting(containerEl).setName("Show date").setDesc("Enable to show the date.").addToggle((toggle) => {
      toggle.setValue(this.plugin.settings.showDate).onChange(async (value) => {
        this.plugin.settings.showDate = value;
        await this.plugin.saveSettings();
        const clockView = this.app.workspace.getLeavesOfType("tokei").find((leaf) => leaf.view instanceof ClockView);
        if (clockView) {
          clockView.view.displayTime();
        }
        dateFormatSetting.settingEl.style.display = value ? "" : "none";
        showWeekAndQuarterSetting.settingEl.style.display = value ? "" : "none";
        weekStartSetting.settingEl.style.display = value ? "" : "none";
        yearStartSetting.settingEl.style.display = value ? "" : "none";
        weekQuarterContainer.style.display = value ? "" : "none";
      });
    });
    showDateSetting.settingEl.classList.add("show-date-settings");
    const dateFormatSetting = new import_obsidian.Setting(containerEl).setName("Date format").setDesc("Default Luxon format is DDD").addText((text) => {
      const textField = text.setPlaceholder("Enter the date format").setValue(this.plugin.settings.dateFormat);
      const hint = document.createElement("small");
      hint.classList.add("small-hint");
      textField.inputEl.after(hint);
      const handleInputDebounced = (0, import_obsidian.debounce)(() => {
      ...
      (button) => button.setIcon("reset").setTooltip("Reset date format").onClick(async () => {
        this.plugin.settings.dateFormat = DEFAULT_SETTINGS.dateFormat;
        await this.plugin.saveSettings();
        this.plugin.view.displayTime();
        const dateFormatInputEl = dateFormatSetting.settingEl.querySelector("input");
        const hint = dateFormatSetting.settingEl.querySelector("small");
        if (dateFormatInputEl instanceof HTMLInputElement) {
          dateFormatInputEl.value = this.plugin.settings.dateFormat;
        }
        if (hint instanceof HTMLElement) {
          hint.textContent = "";
        }
    ...
    const showWeekAndQuarterSetting = new import_obsidian.Setting(containerEl).setName("Show Fiscal Date").setDesc(
      "Enable to show the Fiscal Year & Quarter and the Week of the Quarter."
    ).addToggle((toggle) => {
      toggle.setValue(this.plugin.settings.showWeekAndQuarter).onChange(async (value) => {
        this.plugin.settings.showWeekAndQuarter = value;
        await this.plugin.saveSettings();
        const clockView = this.app.workspace.getLeavesOfType(ClockViewType).find((leaf) => leaf.view instanceof ClockView);
        if (clockView) {
          clockView.view.displayTime();
        }
        weekStartSetting.settingEl.style.display = value ? "" : "none";
    ...
    const weekStartSetting = new import_obsidian.Setting(containerEl).setName("Week Starts On").setDesc(
      "Select the start day of the week. This affects the week of the quarter calculation."
    ).addDropdown((dropdown) => {
      dropdown.addOptions({
        sunday: "Sunday",
        // Option for starting the week on Sunday
        monday: "Monday"
        // Option for starting the week on Monday
      }).setValue(this.plugin.settings.weekStart).onChange(async (value) => {
    ...
    const yearStartSetting = new import_obsidian.Setting(containerEl).setName("Year Starts On").setDesc(
      ...
      dropdown.addOptions(monthOptions).setValue(this.plugin.settings.fiscalYearStart.toString()).onChange(async (value) => {
    ...
    weekQuarterContainer.style.display = this.plugin.settings.showWeekAndQuarter ? "" : "none";
    if (showWeekAndQuarterSetting) {
      weekStartSetting.settingEl.style.display = this.plugin.settings.showWeekAndQuarter ? "" : "none";
      yearStartSetting.settingEl.style.display = this.plugin.settings.showWeekAndQuarter ? "" : "none";
    }
    const showTimezonesSetting = new import_obsidian.Setting(containerEl).setName("Show timezones").setDesc("Enable to show the timezones.").addToggle((toggle) => {
    ...
    const timezoneFormatSetting = new import_obsidian.Setting(containerEl).setName("Timezone Format").setDesc("Default Luxon format is HH:mm EEE").addText((text) => {
      const textField = text.setPlaceholder("Enter the timezone format").setValue(this.plugin.settings.timezoneFormat);
      const hint = document.createElement("small");
      hint.classList.add("small-hint");
      textField.inputEl.after(hint);
      const handleInputDebounced = (0, import_obsidian.debounce)(() => {
      ...
      (button) => button.setIcon("reset").setTooltip("Reset timezone format").onClick(async () => {
        ...
        await this.plugin.saveSettings();
        this.plugin.view.displayTime();
        const timezoneFormatInputEl = timezoneFormatSetting.settingEl.querySelector(
          "input"
        );
        const hint = timezoneFormatSetting.settingEl.querySelector(
          "small"
        );
        if (timezoneFormatInputEl instanceof HTMLInputElement) {
          timezoneFormatInputEl.value = this.plugin.settings.timezoneFormat;
        }
        if (hint instanceof HTMLElement) {
          hint.textContent = "";
        }
        new import_obsidian.Notice("Timezone Format Reset");
      })
    );
    timezoneFormatSetting.settingEl.classList.add(
      "timezone-format-settings"
    );
    const timezonePairsSetting = new import_obsidian.Setting(containerEl).setName("Timezone settings").setDesc(
    ...
    this.plugin.settings.timeZonePairs.forEach((entry, index) => {
      ...
      const handleInput = (0, import_obsidian.debounce)(() => {
      ...
      const handleOffsetChange = (0, import_obsidian.debounce)(() => {
        ...
        if (this.plugin.settings.timeZonePairs.length < 5) {
          ...
          addButton.addEventListener("click", async () => {
            if (this.plugin.settings.timeZonePairs.length < 5) {
              ...
              await this.plugin.saveSettings();
              this.display();
              const newIndex = this.plugin.settings.timeZonePairs.length - 1;
      ...
      } else if (index === this.plugin.settings.timeZonePairs.length - 2 && this.plugin.settings.timeZonePairs.length === 5) {
    ...
    const validateAndDisplayHint = (nameInput, offsetInput) => {
      const newName = nameInput.value.trim();
      if (newName === "") {
    ...
    dateFormatSetting.settingEl.style.display = this.plugin.settings.showDate ? "" : "none";
    timeFormatInput.settingEl.style.display = this.plugin.settings.showDate ? "" : "none";
    timezoneFormatSetting.settingEl.style.display = this.plugin.settings.showTimeZone ? "" : "none";
    timezonePairsSetting.settingEl.style.display = this.plugin.settings.showTimeZone ? "" : "none";
  }
  isValidTimeZoneOffset(offset2) {
    const offsetRegex2 = /^([-+]?((1[0-2]|0?[1-9])(\.\d+)?|0?\.5))|(\+1[0-4](\.\d+)?|0)$/;
    return offsetRegex2.test(offset2);
  }
};
