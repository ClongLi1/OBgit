...
// 圆形标记
timeline += `<div style="width: 10px; height: 10px; background-color: #FF0000; transform: rotate(45deg);"></div>`;
  
// 日期和星期
timeline += `<div style="font-size: 15px; color: #6F6F6F; white-space: nowrap; text-align: center;">`;
timeline += `${currentMonthName} ${currentDay}日<br>${currentDayName}`;
timeline += `</div>`;
timeline += `</div>`;

// 添加月份标签
timeline += `<div style="display: flex; justify-content: space-between; margin-top: 10px; font-size: 15px; color: #6FFF98;">`;
